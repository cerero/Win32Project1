...
int Load_Palette_From_File(char *filename, LPPALETTEENTRY palette)
{
	...
	*/
	...
	for (int index = 0; index < MAX_COLORS_PALETTE; index++)
	{
		...
		fscanf_s(fp_file, "%d %d %d %d", &palette[index].peRed,
...
int Open_Error_File(char *filename, FILE *fp_override)
{
	...
	//ctime_s(timeline,280,&(timebuffer.time));
	sprintf_s(timestring, ".19s.%hu, s", timebuffer.millitm);
	//_ftime(&timebuffer);
...
int Load_Bitmap_File(BITMAP_FILE_PTR bitmap, char *filename)
{
	...
	fin.read((char *)&bitmap->bitmapfileheader, sizeof(BITMAPFILEHEADER));
	...
	fin.read((char *)&bitmap->bitmapinfoheader, sizeof(BITMAPINFOHEADER));

	// now load the color palette if there is one
	if (bitmap->bitmapinfoheader.biBitCount == 8)
	{
		//_lread(file_handle, &bitmap->palette, MAX_COLORS_PALETTE*sizeof(PALETTEENTRY));
		fin.read((char *)&bitmap->palette, MAX_COLORS_PALETTE*sizeof(PALETTEENTRY));
	...
	fin.seekg(-(int)(bitmap->bitmapinfoheader.biSizeImage),ios_base::end);
